...
| Variable | Purpose |
|----------|---------|
| `HF_HOME` | HuggingFace cache directory; points into the PVC so downloads persist across jobs |
| `HF_TOKEN_PATH` | Path to the HuggingFace token file *inside the container*; read at runtime by `huggingface_hub` |
...
| `--enforce-eager` | *(flag)* | Disables CUDA graph compilation; reduces startup from 60–90 min to ~25 min at a small throughput cost |
...
| `unauthenticated requests to HF Hub` | `HF_TOKEN_PATH` not resolving | Verify the token file exists at the path inside the container |
